...
export default function Dashboard() {
  ...
  const countBy = (columnName) => {
    ...
    return Object.entries(result).map(([name, value]) => ({
      name,
      value,
      percent: totalRespondents > 0 ? Number(((value / totalRespondents) * 100).toFixed(1)) : 0,
    }));
  };
  ...
  const over5YearsCount = getCount("Berapa lama perniagaan anda telah beroperasi?", "Lebih daripada 5 tahun");
  const soleProprietorCount = getCount("Berapa orang yang bekerja dalam perniagaan anda, termasuk anda sendiri?", "1 (saya seorang sahaja)");
  ...
  const businessDurationData = countBy("Berapa lama perniagaan anda telah beroperasi?");
    ...
    <main className="dashboard">
      <section className="dashboard-header">
        <div className="header-content">
          <h1>Empowering Women-Led MSMEs in ASEAN to Engage with Safe, Practical AI Tools</h1>
          <h2>
            Survey Respondents Dashboard
            <span style={{ fontSize: '12px', fontWeight: 400, opacity: 0.7 }}> • Live</span>
          </h2>
        </div>
      ...
      <section className="kpi-row">
        <KpiCard title="Gender (Female)" value={getPercent(femaleCount)} subtitle={`${femaleCount} Respondents`} icon="👩" />
        <KpiCard title="Women-Led MSMEs" value={getPercent(womenLedCount)} subtitle={`${womenLedCount} Respondents`} icon="👩‍💼" />
        <KpiCard title="Operating > 5 Years" value={getPercent(over5YearsCount)} subtitle={`${over5YearsCount} Respondents`} icon="📅" />
        <KpiCard title="Sole Proprietor (1 person)" value={getPercent(soleProprietorCount)} subtitle={`${soleProprietorCount} Respondents`} icon="👤" />
        <KpiCard title="Highest Education: SPM" value={getPercent(spmCount)} subtitle={`${spmCount} Respondents`} icon="🎓" />
      </section>
      ...
      <section className="chart-row">
        ...
        <div className="chart-card">
          <HorizontalBarChartCard title="Business Size (Number of People)" data={businessSizeData} />
        </div>
      </section>
      ...
      <section className="chart-row">
        ...
        <div className="chart-card">
          <HorizontalBarChartCard title="Primary Business Sector" data={sectorData} />
        </div>
        <div className="chart-card">
          <DonutChartCard title="Highest Education Level" data={educationData} />
        </div>
      </section>

      <section className="chart-row">
        <div className="chart-card">
          <HorizontalBarChartCard title="Business Operation Duration" data={businessDurationData} />
        </div>
        <div className="chart-card">
          <BarChartCard title="Business Operation by State" data={stateData} />
        </div>
        <div className="chart-card">
